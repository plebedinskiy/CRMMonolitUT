...
Процедура ЗаписатьВXMLИнформациюПоТоварамСкладамИзЗапросаОстатковПоПартиям(XML, ВыборкаПоТоварам)
	...
	XML.ЗаписатьНачалоЭлемента("r");
		...
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоТоварам.НоменклатураМонолит);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоТоварам.УпаковкаМонолит);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ФОРМАТ(ВыборкаПоТоварам.ВНаличииОстаток, "ЧРД=.; ЧГ="));
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ФОРМАТ(ВыборкаПоТоварам.ВНаличииОстаток, "ЧРД=.; ЧГ="));
...
Процедура ЗаписатьВXMLИнформациюПоДокументуИзЗапросаПричинНедопоставки(XML, ВыборкаПоДокументам)
	...
	XML.ЗаписатьНачалоЭлемента("r");
		...
		ЗаписатьВXMLСтрокуСТекст(XML, "f", Формат(Окр(ВыборкаПоДокументам.КоличествоРеализации * ?(ВыборкаПоДокументам.УпаковкаМонолит = "l", 0.03333333333, 1),4), "ЧДЦ=4; ЧРД=.; ЧГ="));
		//ЗаписатьВXMLСтрокуСТекст(XML, "f", Формат(Окр(ВыборкаПоДокументам.РазностьОбъемов * ?(ВыборкаПоДокументам.УпаковкаМонолит = "l", 0.03333333333, 1),4), "ЧДЦ=4; ЧРД=.; ЧГ="));
...
Процедура ЗаписатьВXMLИнформациюПоКонтрагентуПартнеруИзЗапросаГрафикаДоставки(XML, ВыборкаПоКонтрагентамПартнерам)
	...
	XML.ЗаписатьНачалоЭлемента("r");
		...
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.ДатаОтчетаСтрокой);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", "");
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День1);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День2);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День3);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День4);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День5);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День6);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.День7);
...
Процедура ВыполнитьЗаписьЗапросаСпискаБлокированныхКлиентовВРегистрФоном(МассивСтруктурШлюзов) Экспорт
	...
	Запрос.Текст = Mon_Общий.ВернутьТекстЗапросаСпискаБлокированныхКлиентовСЕГАИС();
...
Процедура ЗаписатьВXMLИнформациюПоКонтрагентуПартнеруИзЗапросаСпискаБлокированныхКлиентов(XML, ВыборкаПоКонтрагентамПартнерам)
	...
	XML.ЗаписатьНачалоЭлемента("r");
		...
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.ПартнерКод);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.ДатаОтчетаСтрокой);
		ЗаписатьВXMLСтрокуСТекст(XML, "f", ВыборкаПоКонтрагентамПартнерам.ТипБлокировки);
