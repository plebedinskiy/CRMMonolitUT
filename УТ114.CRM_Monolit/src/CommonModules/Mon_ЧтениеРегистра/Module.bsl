	...
	Для Каждого Строка из СписокДокументов Цикл
		...
		Для Каждого СтрокаТовары Из Строка.CRMOrderLine Цикл
			...
			Иначе
				...
				Попытка
					...
					Если СрокГодностиМассивом.Количество() = 3 Тогда
			...
			Для Каждого Строка Из НовыйДокумент.Товары Цикл
				...
				Если ТЗ.Итог("КоличествоОборот") >= ОстатокДляСписания Тогда
					Для Каждого СтрокаСерииИРелизации Из ТЗ Цикл
						...
						//Для первой строки корректируем текущую, а для следующей добавляем новую
						Если ОстатокДляСписания = Строка.Количество Тогда
							СтрокаТоварыДляРаботы = Строка;
						Иначе
							СтрокаТоварыДляРаботы = НовыйДокумент.Товары.Добавить();
						...
						Если ОстатокДляСписания <= 0 Тогда
							Прервать;
						КонецЕсли;
						Если РезультатПроверкиВозврата1с = Истина Тогда
		...
		Попытка
			...
			Иначе
				...
				НовыйДокумент.Записать(?(СтруктураРеквизитовДокумента.ЕстьКритическиеОшибки ИЛИ ГлобальныйРезультатПроверкиВозврата = Ложь ИЛИ РезультатПроверкиВозврата1с = Ложь, РежимЗаписиДокумента.Запись, РежимЗаписиДокумента.Проведение));
			КонецЕсли;
		...
		Если ЭтоЗаказ И НЕ ТипДокументаЗагрузки = "ПеремещениеТоваров" И НЕ СтруктураРеквизитовДокумента.ЕстьКритическиеОшибки Тогда
			...
			Исключение
				...
				НовыйДокумент.Записать(РежимЗаписиДокумента.Запись);
			КонецПопытки;
		ИначеЕсли Ложь И ЭтоВозврат И (СтруктураРеквизитовДокумента.ЕстьКритическиеОшибки = Ложь И ГлобальныйРезультатПроверкиВозврата = Истина И РезультатПроверкиВозврата1с = Истина ) Тогда
