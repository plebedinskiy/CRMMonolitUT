	...
	Для Каждого Элемент Из ПараметрыФоновогоЗадания Цикл  
		...
		Для Каждого СтрокаИзТЗ Из ТЗНеобработанные Цикл
			...
			Попытка
				...
				ИначеЕсли Элемент.ВидОбмена = Перечисления.Mon_ВидыОбменов.CRMVendReceiptExParts Тогда
						
						
						ЗагрузитьПоступленияПоСпискуДокументовИзСоответствияДанныхXML(Элемент, СоответствиеДанныхXML);
...
Процедура ЗагрузитьЗаказыИРеализацииПоСпискуДокументов(СтруктураШлюза, СоответствиеДанныхXML)
	...
	Для Каждого Строка из СписокДокументов Цикл
		...
		ИначеЕсли ЭтоВозврат Тогда
			...
			Для Каждого Строка Из НовыйДокумент.Товары Цикл
				...
				Если ТЗ.Итог("КоличествоОборот") >= ОстатокДляСписания И ТЗ.Количество() <> 0 Тогда
					Для Каждого СтрокаСерииИРелизации Из ТЗ Цикл
						...
						//*- Плохо подбирается 
						//Если (СтрокаСерииИРелизации.Организация = НовыйДокумент.Организация И СтрокаСерииИРелизации.Приоритет <> 3) ИЛИ 
						//Строка.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
						Если Строка.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
				...
				Иначе
					...
					Если НЕ Строка.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
						...
						Если ЗначениеЗаполнено(Строка.Номенклатура) Тогда
							...
							СерияДляДобавления.Номер = Справочники.СерииНоменклатуры.ВычислитьМаксимальныйНомерСерии(Строка.Номенклатура.ВидНоменклатуры);
							СерияДляДобавления.ДатаПроизводства = '00010101';
			...
			//НовыйДокумент.Серии.Свернуть("Серия, Номенклатура","Количество");
		...
		Если ЭтоЗаказ И НЕ ТипДокументаЗагрузки = "ПеремещениеТоваров" И НЕ СтруктураРеквизитовДокумента.ЕстьКритическиеОшибки Тогда
			...
			ДокОбъект.СостояниеЗаполненияМногооборотнойТары = НовыйДокумент.Ссылка.СостояниеЗаполненияМногооборотнойТары;
			Попытка
...
Процедура ЗагрузитьПоступленияПоСпискуДокументовИзСоответствияДанныхXML(СтруктураШлюза, СоответствиеДанныхXML)


	ТЗДокументов = Новый ТаблицаЗначений;
	
	ТЗДокументов.Колонки.Добавить( "DocumentDate", 				Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	ТЗДокументов.Колонки.Добавить( "DocumentNumber", 			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "Document_Object", 			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "DocumentTypeId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "SrcCRMDbId", 				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "WareHouseId",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗДокументов.Колонки.Добавить( "IsConsignment", 			Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗДокументов.Колонки.Добавить( "DocumentBarCode",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 200)); 
	ТЗДокументов.Колонки.Добавить( "AutoTruckCompanyName",  	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "AutoTruckCompanyTaxCode",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "AutoTruckCompanyTaxReason", Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "AutoNumber",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "AutoDriverPersonPassport",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "AutoDriverName",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "SenderCompanyTaxReason",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "SenderCompanyTaxCode", 		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "SenderCompanyName",   		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	
	ТЗДокументов.Колонки.Добавить( "ShipCompanyTaxReason",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 200)); 
	ТЗДокументов.Колонки.Добавить( "ShipCompanyTaxCode",  		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ShipCompanyName",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "ReceivCompanyTaxReason", 	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ReceivCompanyTaxCode",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "ReceivCompanyName",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "SrcDocumentNumber",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "MoveDate", 					Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	ТЗДокументов.Колонки.Добавить( "FixNotificationNum",   		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));	
	ТЗДокументов.Колонки.Добавить( "FixNotificationDate",		Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	
	ТЗДокументов.Колонки.Добавить( "IdTTN",						Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "ExtCompanyId",  			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ExtAddressId",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DespatchCompanyId", 		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "DespatchAddressId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DespatchCompanyName",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DespatchAddressName",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));	
	ТЗДокументов.Колонки.Добавить( "DespatchLocation",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "TaxCode",  					Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "TaxReason",					Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "AddressTaxReason", 			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "DTOrderTimeMin",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DTOrderTimeMax",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	
	ТЗДокументов.Колонки.Добавить( "XDocPointLat",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 50));	
	ТЗДокументов.Колонки.Добавить( "XDocPointLon",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 50)); 
	ТЗДокументов.Колонки.Добавить( "XDocDTOrderDay",  			Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	ТЗДокументов.Колонки.Добавить( "XDocOrderTimeMin",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "XDocOrderTimeMax", 			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "XDocWeight",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "XDocServDuration",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "XDocServDurationFix",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));	
	ТЗДокументов.Колонки.Добавить( "XDocWHLocation",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "XDocWHAddressName",  		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ExtDocumentNumber",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 300));

		
	ТЗСтрокиДокументов = Новый ТаблицаЗначений;
	ТЗСтрокиДокументов.Колонки.Добавить( "Document_Object",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиДокументов.Колонки.Добавить( "SrcCRMDbId",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиДокументов.Колонки.Добавить( "LineNumber",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "WareId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиДокументов.Колонки.Добавить( "UnitId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиДокументов.Колонки.Добавить( "UnitFactor",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "Price",			Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 4));
	ТЗСтрокиДокументов.Колонки.Добавить( "Quantity",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "QuantityL",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "ProductKind",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиДокументов.Колонки.Добавить( "Alcohol",			Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 4));
	
	ТЗСтрокиПартий = Новый ТаблицаЗначений; 
	ТЗСтрокиПартий.Колонки.Добавить( "Document_Object",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "SrcCRMDbId",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "PartNumber",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "LineNumber",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 3));
	ТЗСтрокиПартий.Колонки.Добавить( "WareId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиПартий.Колонки.Добавить( "UnitId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиПартий.Колонки.Добавить( "ExpirationDate",	Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	ТЗСтрокиПартий.Колонки.Добавить( "ProductionDate",	Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	ТЗСтрокиПартий.Колонки.Добавить( "PalletBarCode",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиПартий.Колонки.Добавить( "Quantity",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 4));
	ТЗСтрокиПартий.Колонки.Добавить( "FactoryTaxReason",Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "FactoryTaxCode",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "FactoryName",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "OriginCountry",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "GTDNumber",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISCode",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "MultiPackRatio",	Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиПартий.Колонки.Добавить( "WareHouseRegId",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "WareHouseBRegId",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "CNLWareId",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "CNLWareName",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "WHUTMIdentity",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISAlcCode",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISFullName",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 500));
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISWbUnitType",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
		
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerClientRegId",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerFullName",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 500));	
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerShortName",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerCountry",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerDescription",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerINN",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerKPP",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));		
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerRegionCode",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	
	ТЗСтрокиПартий.Колонки.Добавить( "PackageQuantityAL",		Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 4));
	ТЗСтрокиПартий.Колонки.Добавить( "EANCodeId",				Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "WareHouseAnalytLotIntId",	Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 4));			
	...
	СписокДокументов = СоответствиеДанныхXML["CRMVendReceipt"];
	...
	Для Каждого Док из СписокДокументов Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Док);
		Для Каждого СтрокаИзПоступления Из Док.CRMVendReceiptLine Цикл
			СтрокаИзПоступленияВТЗ = ТЗСтрокиДокументов.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаИзПоступленияВТЗ, Док);
			ЗаполнитьЗначенияСвойств(СтрокаИзПоступленияВТЗ, СтрокаИзПоступления);		
		КонецЦикла;
		Для Каждого СтрокаИзПоступления Из Док.CRMVendReceiptParts Цикл
			СтрокаИзПоступленияВТЗ = ТЗСтрокиПартий.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаИзПоступленияВТЗ, Док);
			ЗаполнитьЗначенияСвойств(СтрокаИзПоступленияВТЗ, СтрокаИзПоступления);		
		КонецЦикла;	
	...
	СписокПаллет = СоответствиеДанныхXML["CRMVendReceiptPallets"];
	Для Каждого Палета из СписокПаллет Цикл
		
		НоваяСтрока = ТЗПалет.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Палета);
...
Процедура ДобавитьПодобраннуюТаруВТаблицу(СтрокиДерева, ТаблицаТары)
	...
	Для Каждого ТекСтрока Из СтрокиДерева Цикл
		...
		Если (ТекСтрока.Выбрана = Истина Или (Не ТекСтрока.Выбрана = Истина И ?(ТекСтрока.КоличествоТарыВДокументе = Неопределено,0,ТекСтрока.КоличествоТарыВДокументе) > 0))
			...
			НоваяСтрока.Количество = ?(ТекСтрока.Выбрана = Истина, ТекСтрока.ПодобранноеКоличествоТары, ТекСтрока.КоличествоТарыВДокументе);
			НоваяСтрока.Склад = ТекСтрока.Склад;
			НоваяСтрока.Дата = ТекСтрока.Дата;
