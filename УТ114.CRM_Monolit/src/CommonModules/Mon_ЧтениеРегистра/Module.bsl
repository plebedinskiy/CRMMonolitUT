	...
	Для Каждого Строка из СписокДокументов Цикл
		...
		ИначеЕсли ЭтоВозврат Тогда
			...
			Для Каждого Строка Из НовыйДокумент.Товары Цикл
				...
				ТЗ = ВернутьТаблицуЗначенийРеализацийСерийИДокументовРеализаций(ПартнерДляДокумента, Строка.Номенклатура, СрокГодности, ОрганизацияДляПоискаРеализаций);
				ОстатокДляСписания = Строка.Количество;
				Если ТЗ.Итог("КоличествоОборот") >= ОстатокДляСписания Тогда
					Для Каждого СтрокаСерииИРелизации Из ТЗ Цикл
						...
						Если НЕ Строка.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
							...
							СтрСерии.Серия 					= СтрокаСерииИРелизации.Серия;
...
Функция ВернутьТаблицуЗначенийРеализацийСерийИДокументовРеализаций(ПартнерДляДокумента, Номенклатура, СрокГодностиДатой, ОрганизацияДляПоискаРеализаций)
	...
	Иначе
		...
		|	ВЫБОР КОГДА ВТ_ДокументыРеализации.Организация = &ОрганизацияДляПоискаРеализаций ТОГДА 1 ИНАЧЕ 2 КОНЕЦ КАК Приоритет,
