...
Процедура ЗагрузитьЗаказыИРеализацииПоСпискуДокументовИзСоответствияДанныхXML(СтруктураШлюза, СоответствиеДанныхXML)
	...
	|	ЕдиницаИзмеренияКоличестваШтук.Значение,
	|	ЕдиницаИзмеренияРазливнойПродукции.Значение,
	|	РозливЛитры.Ссылка
	|ИЗ
	|	ВТ_ТЗСтрокиДокументов КАК ВТ_ТЗСтрокиДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СоответствияНоменклатуры КАК ВТ_СоответствияНоменклатуры
	|		ПО ТЗСтрокиДокументов.WareId = ВТ_СоответствияНоменклатуры.НоменклатураМонолит
	|		И ТЗСтрокиДокументов.UnitId = ВТ_СоответствияНоменклатуры.УпаковкаМонолит
	...
	|		ПО ПО (УпаковкиЕдиницыИзмерения.Код = ""112"")";
