	...
	Для Каждого Строка из СписокДокументов Цикл
		...
		Для Каждого СтрокаТовары Из Строка.CRMOrderLine Цикл
			...
			Иначе
				...
				Попытка
					...
					Если ГлобальныйРезультатПроверкиВозврата = Истина И РезультатПроверки = Ложь Тогда
		...
		ИначеЕсли ЭтоВозврат Тогда
			...
			Для Каждого Строка Из НовыйДокумент.Товары Цикл
				...
				//*-Добавить проверку на возврат.
				Если СоответствиеПроверокСтрокВозврата.Получить(Строка) = Ложь Тогда
				...
				Если ТЗ.Итог("КоличествоОборот") >= ОстатокДляСписания Тогда
					Для Каждого СтрокаСерииИРелизации Из ТЗ Цикл
						...
						Если ОрганизацияДляПоискаРеализаций = "" И НовыйДокумент.Организация <> СтрокаСерииИРелизации.Документ.Организация Тогда
...
Функция ВернутьТаблицуЗначенийРеализацийСерийИДокументовРеализаций(ПартнерДляДокумента, Номенклатура, СрокГодностиДатой, ОрганизацияДляПоискаРеализаций)
	...
	Иначе
		...
		|	ДвиженияСерийТоваровОбороты.Документ.Дата КАК Период,
		...
		|   И (Документ.Организация = &ОрганизацияДляПоискаРеализаций ИЛИ &ОрганизацияДляПоискаРеализаций = """")
		...
		|		И (ВозвратТоваровОтКлиентаТовары.Ссылка.Организация = &ОрганизацияДляПоискаРеализаций ИЛИ &ОрганизацияДляПоискаРеализаций = """")
