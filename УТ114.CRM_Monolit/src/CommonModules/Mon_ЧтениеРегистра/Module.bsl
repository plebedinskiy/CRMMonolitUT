	...
	Для Каждого Строка из СписокДокументов Цикл
		...
		Для Каждого СтрокаТовары Из Строка.CRMOrderLine Цикл
			...
			Иначе
				...
				СтрТовары.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзДокументаПродажи;
				...
				Попытка
					...
					Если РезультатПроверки = Истина Тогда 
						...
						ТЗ = ВернутьТаблицуЗначенийРеализацийСерийИДокументовРеализаций(ПартнерДляДокумента, СтрТовары.Номенклатура, СрокГодностиДатой);
						ОстатокДляСписания = СтрТовары.Количество;
						Если ТЗ.Итог("КоличествоОборот") <= ОстатокДляСписания Тогда
							Для Каждого СтрокаСерииИРелизации Из ТЗ Цикл
								...
								Если ОстатокДляСписания = СтрТовары.Количество Тогда
									СтрокаТоварыДляРаботы = СтрТовары;
								Иначе
									СтрокаТоварыДляРаботы = НовыйДокумент.Товары.Добавить();
									ЗаполнитьЗначенияСвойств(СтрокаТоварыДляРаботы, СтрокаТовары);	
								КонецЕсли;
								
								
								Если ОстатокДляСписания >= СтрокаСерииИРелизации.КоличествоОборот Тогда
									//*-Добавляем серию
									СтрСерии 						= НовыйДокумент.Серии.Добавить();
									СтрСерии.Серия 					= СтрокаСерииИРелизации.Серия;
									СтрСерии.Номенклатура 			= СтрТовары.Номенклатура;
									СтрСерии.Количество 			= СтрокаСерииИРелизации.КоличествоОборот;
									
									//исправляем количество в ТЗ Товары
									СтрокаТоварыДляРаботы.Количество 			= СтрокаСерииИРелизации.КоличествоОборот;
									СтрокаТоварыДляРаботы.КоличествоУпаковок 	= СтрокаСерииИРелизации.КоличествоОборот;
									...
									Если НовыйДокумент.Организация <> СтрокаСерииИРелизации.Документ.Организация Тогда
								...
								ОстатокДляСписания = ОстатокДляСписания - СтрСерии.Количество;
						...
						КонецЕсли;
					КонецЕсли;
					КомментарийКВозврату = КомментарийКВозврату + "Стр.№" + СтрокаТовары.LineNumber +  "(" + НоменклатураМонолит + ") - "+ СрокГодности + ?(РезультатПроверки, "Одобрен", "Не одобрен") + Символы.ПС;
...
Функция ВернутьТаблицуЗначенийРеализацийСерийИДокументовРеализаций(ПартнерДляДокумента, Номенклатура, СрокГодностиДатой)
	...
	|УПОРЯДОЧИТЬ ПО
	|	ДвиженияСерийТоваровОбороты.Период";
	Запрос.УстановитьПараметр("Получатель", 	ПартнерДляДокумента);
	Запрос.УстановитьПараметр("ГоденДо", 		СрокГодностиДатой);
