...
Процедура ЗагрузитьПоступленияПоСпискуДокументовИзСоответствияДанныхXML(СтруктураШлюза, СоответствиеДанныхXML)
	...
	Запрос = Новый Запрос(Mon_Общий.ВернутьТекстЗапросаПоступлений());
	
	Запрос.УстановитьПараметр("ТЗДокументов", 			ТЗДокументов);
	Запрос.УстановитьПараметр("ТЗСтрокиДокументов", 	ТЗСтрокиДокументов);
	Запрос.УстановитьПараметр("ТЗСтрокиПартий",			ТЗСтрокиПартий);
	
	ВыборкаПоТЧТовары = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);

	Пока ВыборкаПоТЧТовары.Следующий() Цикл
		
		//*-Справить все транизациями
		ДокументПоступления = ВыборкаПоТЧТовары.Ссылка.ПолучитьОбъект();
		
		ВыборкаТоваровПоТЧТовары = ВыборкаПоТЧТовары.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
		ДокументПоступления.Товары.Очистить();
		ДокументПоступления.Серии.Очистить();
		Пока ВыборкаТоваровПоТЧТовары.Следующий() Цикл
			Строка = ДокументПоступления.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(Строка, ВыборкаТоваровПоТЧТовары);
			
			Строка.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(Строка.Сумма, Истина, УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(Строка.Номенклатура.СтавкаНДС));
			Строка.СуммаСНДС = СтрокаТоварыДляРаботы.Сумма;

			Строка.СебестоимостьБезНДС = Строка.Сумма - Строка.СуммаНДС;
			
				
			//*-Заполнить все, что касается НДС
			ВыборкаПоТЧСерий = ВыборкаТоваровПоТЧТовары.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаПоТЧСерий.Следующий() Цикл
				Строка = ДокументПоступления.Серии.Добавить();
				ЗаполнитьЗначенияСвойств(Строка, ВыборкаПоТЧСерий);
			КонецЦикла;
		...
		Запись 			= РегистрыСведений.ДополнительныеСведения.СоздатьМенеджерЗаписи();
		Запись.Объект 	= ДокументПоступления.Ссылка;
		Запись.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "ПровереноМонолитПоступлениеТоваровУслуг");
