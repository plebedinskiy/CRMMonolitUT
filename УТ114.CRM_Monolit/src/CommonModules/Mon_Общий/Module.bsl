...
Функция ВернутьТекстЗапросаНаПолучениеПартнера() Экспорт
	...
	ТекстЗапроса =	"ВЫБРАТЬ
		...
		|	ВЫРАЗИТЬ(ДополнительныеСведения.Объект КАК Справочник.Партнеры).Родитель КАК ГоловнойПартнер,
		...
		|	ГДЕ
		|		ДополнительныеСведения.Объект Ссылка Справочник.Партнеры И
		|		ДополнительныеСведения.Свойство.Имя = ""КодМонолитОчаковоПартнеры""
		|		И ДополнительныеСведения.Значение = &КодТорговойТочки";
...
Функция ВернутьТекстЗапросаГрафикаДоставки() Экспорт
	...
	|ИЗ
	|	ВТ_ТЗ КАК ВТ_ТЗ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Mon_ШлюзыОрганизации.ТорговыеПредставители КАК
	|			Mon_ШлюзыОрганизацииТорговыеПредставители
	...
	|ПОМЕСТИТЬ ВТ_СобранныеКонтрагенты
	|ИЗ
	|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	...
	|	УсловияОбслуживанияПартнеровТорговымиПредставителями.Владелец КАК Владелец,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 0
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ1,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 1
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ2,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 2
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ3,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 3
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ4,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 4
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ5,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 5
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ6,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.ДеньНедели.Порядок = 6
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ) КАК ДЕНЬ7,
	|	ВТ_СобранныеКонтрагенты.ШлюзОрганизации КАК ШлюзОрганизации,
	|	ВТ_СобранныеКонтрагенты.Шлюз КАК Шлюз,
	|	ВТ_СобранныеКонтрагенты.ВидОбмена КАК ВидОбмена,
	|	ВТ_СобранныеКонтрагенты.Пароль КАК Пароль,
	|	ВТ_СобранныеКонтрагенты.ФлагУдаления КАК ФлагУдаления,
	|	ВТ_СобранныеКонтрагенты.ДатаОтчетаСтрокой КАК ДатаОтчетаСтрокой,
	|	ВТ_СобранныеКонтрагенты.ДатаОтчета КАК ДатаОтчета,
	|	ВТ_СобранныеКонтрагенты.ПартнерКод КАК ПартнерКод,
	|	ВТ_СобранныеКонтрагенты.КонтрагентИНН КАК КонтрагентИНН,
	|	ВТ_СобранныеКонтрагенты.КодТорговойТочки КАК КодТорговойТочки
	|ИЗ
	|	Справочник.УсловияОбслуживанияПартнеровТорговымиПредставителями.ГрафикПосещения КАК
	|		УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УсловияОбслуживанияПартнеровТорговымиПредставителями КАК
	|			УсловияОбслуживанияПартнеровТорговымиПредставителями
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СобранныеКонтрагенты КАК ВТ_СобранныеКонтрагенты
	|			ПО (УсловияОбслуживанияПартнеровТорговымиПредставителями.Владелец = ВТ_СобранныеКонтрагенты.Партнер)
	|		ПО (УсловияОбслуживанияПартнеровТорговымиПредставителямиГрафикПосещения.Ссылка = УсловияОбслуживанияПартнеровТорговымиПредставителями.Ссылка)
	|СГРУППИРОВАТЬ ПО
	|	УсловияОбслуживанияПартнеровТорговымиПредставителями.Владелец,
	|	ВТ_СобранныеКонтрагенты.ШлюзОрганизации,
	|	ВТ_СобранныеКонтрагенты.Шлюз,
	|	ВТ_СобранныеКонтрагенты.ВидОбмена,
	|	ВТ_СобранныеКонтрагенты.Пароль,
	|	ВТ_СобранныеКонтрагенты.ФлагУдаления,
	|	ВТ_СобранныеКонтрагенты.ДатаОтчетаСтрокой,
	|	ВТ_СобранныеКонтрагенты.ДатаОтчета,
	|	ВТ_СобранныеКонтрагенты.ПартнерКод,
	|	ВТ_СобранныеКонтрагенты.КонтрагентИНН,
	|	ВТ_СобранныеКонтрагенты.КодТорговойТочки
	|ИТОГИ
	|	МАКСИМУМ(Шлюз),
	|	МАКСИМУМ(Пароль),
	|	МАКСИМУМ(ДатаОтчета),
...
Функция ВернутьТекстЗапросаСпискаБлокированныхКлиентовСЕГАИС() Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|  ВЫРАЗИТЬ(ТТНИсходящаяЕГАИС.ДокументОснование КАК Документ.РеализацияТоваровУслуг).Контрагент КАК Контрагент,
	|  ВЫРАЗИТЬ(ТТНИсходящаяЕГАИС.ДокументОснование КАК Документ.РеализацияТоваровУслуг).Организация КАК Организация,
	|  ВЫРАЗИТЬ(ТТНИсходящаяЕГАИС.ДокументОснование КАК Документ.РеализацияТоваровУслуг).Партнер КАК Ссылка
	|ПОМЕСТИТЬ ВТ_БлокированныеПартнеры
	|ИЗ
	|  Документ.ТТНИсходящаяЕГАИС КАК ТТНИсходящаяЕГАИС
	|   
	|ГДЕ
	|  ТТНИсходящаяЕГАИС.Дата <= ДОБАВИТЬКДАТЕ(&ДатаОтчета, ДЕНЬ, -14)
	|  И ТТНИсходящаяЕГАИС.Проведен
 	| И ТТНИсходящаяЕГАИС.Ссылка В
	|      (ВЫБРАТЬ
	|        СтатусыДокументовЕГАИС.Документ КАК Документ
	|      ИЗ
	|        РегистрСведений.СтатусыДокументовЕГАИС КАК СтатусыДокументовЕГАИС
	|      ГДЕ
	|        СтатусыДокументовЕГАИС.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиТТНИсходящейЕГАИС.ОбрабатываетсяКлиентом))
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_ТорговыеПредставители.ШлюзОрганизации КАК ШлюзОрганизации,
	...
	|	ВТ_СоответствияТорговыхТочек.КодТорговойТочки КАК КодТорговойТочки,
	|	ВТ_БлокированныеПартнеры.Контрагент КАК Контрагент
	|ПОМЕСТИТЬ ВТ_ПодготовленныйОтчетПоЕГАИС
	|ИЗ
	|	ВТ_СоответствияТорговыхТочек КАК ВТ_СоответствияТорговыхТочек
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_БлокированныеПартнеры КАК ВТ_БлокированныеПартнеры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТорговыеПредставители КАК ВТ_ТорговыеПредставители
	|			ПО ВТ_БлокированныеПартнеры.Организация = ВТ_ТорговыеПредставители.Организация
	|		ПО ВТ_СоответствияТорговыхТочек.Партнер = ВТ_БлокированныеПартнеры.Ссылка
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	""1"" КАК ТипБлокировки
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(&ДатаОтчета, ДатаПлановогоПогашения < &ДатаОтчета) КАК
	|		РасчетыСКлиентамиПоСрокам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТорговыеПредставители КАК ВТ_ТорговыеПредставители
	|		ПО (РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам.Договор.Организация = ВТ_ТорговыеПредставители.Организация
	|		ИЛИ
	|			РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам.Договор.Организация = ВТ_ТорговыеПредставители.ОрганизацияФ2)
	|		И
	|			РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам.Договор.Менеджер = ВТ_ТорговыеПредставители.ТорговыйПредставитель
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СоответствияТорговыхТочек КАК ВТ_СоответствияТорговыхТочек
	|		ПО РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам.Партнер = ВТ_СоответствияТорговыхТочек.Партнер
	|ГДЕ
	|	РасчетыСКлиентамиПоСрокам.ДолгОстаток > 0
	...
	|
	|ВЫБРАТЬ
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.ШлюзОрганизации,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.Шлюз,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.ВидОбмена,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.Пароль,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.ФлагУдаления,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.ДатаОтчетаСтрокой,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.ДатаОтчета,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.Контрагент.ИНН,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.Партнер.Код,
	|	ВТ_ПодготовленныйОтчетПоЕГАИС.КодТорговойТочки,
	|	""5""
	|ИЗ
	|	ВТ_ПодготовленныйОтчетПоЕГАИС КАК ВТ_ПодготовленныйОтчетПоЕГАИС
	|ИТОГИ
	|	МАКСИМУМ(Шлюз),
	|	МАКСИМУМ(ВидОбмена),
	|	МАКСИМУМ(Пароль),
...
Функция ВернутьТекстЗапросаПричинНедопоставки() Экспорт
	...
	|ГДЕ
	|	(Mon_СоответствияНоменклатуры.Упаковка.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(ПЕРЕЧИСЛЕНИЕ.ТипыИзмеряемыхВеличин.Объем)
	|	И Mon_СоответствияНоменклатуры.Коэффициент = 0.0333)
	|	ИЛИ
	|		(Mon_СоответствияНоменклатуры.Упаковка.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(ПЕРЕЧИСЛЕНИЕ.ТипыИзмеряемыхВеличин.КоличествоШтук)
	...
	|	РАЗНОСТЬДАТ(ВТ_ЗаказыСгруппированные.ДатаОтгрузки, ЕСТЬNULL(ВТ_РеализацииСвернутые.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|		ДЕНЬ) КАК РазностьДат,
	|	ВЫБОР
	...
	|		КОГДА НЕ РАЗНОСТЬДАТ(ВТ_ЗаказыСгруппированные.ДатаОтгрузки, ЕСТЬNULL(ВТ_РеализацииСвернутые.Дата, ДАТАВРЕМЯ(1, 1,
	|			1)), ДЕНЬ) = 0
	|		И ВТ_ПричинаНедопоставкиЗаказКлиента.ЗаказКлиента ЕСТЬ NULL
	|			ТОГДА ""3""
	|		КОГДА НЕ РАЗНОСТЬДАТ(ВТ_ЗаказыСгруппированные.ДатаОтгрузки, ЕСТЬNULL(ВТ_РеализацииСвернутые.Дата, ДАТАВРЕМЯ(1, 1,
	...
	|		ПО ВТ_ЗаказыСгруппированные.Номенклатура = ВТ_Сопоставления.Номенклатура
	|ГДЕ
	|	НЕ ВТ_ЗаказыСгруппированные.Количество - ЕСТЬNULL(ВТ_РеализацииСгруппированные.Количество, 0) = 0
	|	ИЛИ НЕ РАЗНОСТЬДАТ(ВТ_ЗаказыСгруппированные.ДатаОтгрузки, ЕСТЬNULL(ВТ_РеализацииСвернутые.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|		ДЕНЬ) = 0
	|	ИЛИ ВТ_РеализацииСвернутые.Ссылка ЕСТЬ NULL
	|ИТОГИ
	|	МАКСИМУМ(Шлюз),
	|	МАКСИМУМ(ВидОбмена),
	|	МАКСИМУМ(Пароль),
...
Функция ВернутьТекстЗапросаДвижений() Экспорт
	...
	Возврат "ВЫБРАТЬ
		...
		|ГДЕ
		|	ДополнительныеСведения.Объект ССЫЛКА Справочник.Партнеры
		|	И ДополнительныеСведения.Свойство.Имя = ""КодШлюзаОчаковоПартнеры""
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			И (РеализацияТоваровУслугТовары.КлючСвязи = ВТ_Скидки.КлючСвязи)
...
Функция ВернутьТЗДокументов() Экспорт


	ТЗДокументов = Новый ТаблицаЗначений;
	
	ТЗДокументов.Колонки.Добавить( "DocumentDate", 				УстановитьТипДанных( Тип("Дата")));
	ТЗДокументов.Колонки.Добавить( "DocumentNumber", 			УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "Document_Object", 			УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "DocumentTypeId",			УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "SrcCRMDbId", 				УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "WareHouseId",				УстановитьТипДанных( Тип("Строка"), 30));
	ТЗДокументов.Колонки.Добавить( "IsConsignment", 			УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗДокументов.Колонки.Добавить( "DocumentBarCode",			УстановитьТипДанных( Тип("Строка"), 200)); 
	ТЗДокументов.Колонки.Добавить( "AutoTruckCompanyName",  	УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "AutoTruckCompanyTaxCode",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "AutoTruckCompanyTaxReason", УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "AutoNumber",				УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "AutoDriverPersonPassport",	УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "AutoDriverName",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "SenderCompanyTaxReason",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "SenderCompanyTaxCode", 		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗДокументов.Колонки.Добавить( "SenderCompanyName",   		УстановитьТипДанных( Тип("Строка"), 300)); 
	
	ТЗДокументов.Колонки.Добавить( "ShipCompanyTaxReason",		УстановитьТипДанных( Тип("Строка"), 200)); 
	ТЗДокументов.Колонки.Добавить( "ShipCompanyTaxCode",  		УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ShipCompanyName",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "ReceivCompanyTaxReason", 	УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ReceivCompanyTaxCode",		УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "ReceivCompanyName",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "SrcDocumentNumber",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "MoveDate", 					УстановитьТипДанных( Тип("Дата")));
	ТЗДокументов.Колонки.Добавить( "FixNotificationNum",   		УстановитьТипДанных( Тип("Строка"), 300));	
	ТЗДокументов.Колонки.Добавить( "FixNotificationDate",		УстановитьТипДанных( Тип("Дата")));
	
	ТЗДокументов.Колонки.Добавить( "IdTTN",						УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗДокументов.Колонки.Добавить( "ExtCompanyId",  			УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ExtAddressId",				УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DespatchCompanyId", 		УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "DespatchAddressId",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DespatchCompanyName",		УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DespatchAddressName",		УстановитьТипДанных( Тип("Строка"), 300));	
	ТЗДокументов.Колонки.Добавить( "DespatchLocation",			УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "TaxCode",  					УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "TaxReason",					УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "AddressTaxReason", 			УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "DTOrderTimeMin",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "DTOrderTimeMax",			УстановитьТипДанных( Тип("Строка"), 300));
	
	ТЗДокументов.Колонки.Добавить( "XDocPointLat",				УстановитьТипДанных( Тип("Строка"), 50));	
	ТЗДокументов.Колонки.Добавить( "XDocPointLon",				УстановитьТипДанных( Тип("Строка"), 50)); 
	ТЗДокументов.Колонки.Добавить( "XDocDTOrderDay",  			УстановитьТипДанных( Тип("Дата")));
	ТЗДокументов.Колонки.Добавить( "XDocOrderTimeMin",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "XDocOrderTimeMax", 			УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "XDocWeight",				УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "XDocServDuration",			УстановитьТипДанных( Тип("Строка"), 300));
	ТЗДокументов.Колонки.Добавить( "XDocServDurationFix",		УстановитьТипДанных( Тип("Строка"), 300));	
	ТЗДокументов.Колонки.Добавить( "XDocWHLocation",			УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "XDocWHAddressName",  		УстановитьТипДанных( Тип("Строка"), 300)); 
	ТЗДокументов.Колонки.Добавить( "ExtDocumentNumber",			УстановитьТипДанных( Тип("Строка"), 300));

	Возврат ТЗДокументов;

КонецФункции

Функция ВернутьТЗСтрокиДокументов() Экспорт

	ТЗСтрокиДокументов = Новый ТаблицаЗначений;
	ТЗСтрокиДокументов.Колонки.Добавить( "DocumentDate", 	УстановитьТипДанных( Тип("Дата")));
	ТЗСтрокиДокументов.Колонки.Добавить( "DocumentNumber", 	УстановитьТипДанных( Тип("Строка"), 100)); 
	ТЗСтрокиДокументов.Колонки.Добавить( "Document_Object",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиДокументов.Колонки.Добавить( "SrcCRMDbId",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиДокументов.Колонки.Добавить( "LineNumber",		УстановитьТипДанных( Тип("Число"), 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "WareId",			УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиДокументов.Колонки.Добавить( "UnitId",			УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиДокументов.Колонки.Добавить( "UnitFactor",		УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "Price",			УстановитьТипДанных( Тип("Число"), 15, 4));
	ТЗСтрокиДокументов.Колонки.Добавить( "Quantity",		УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "QuantityL",		УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиДокументов.Колонки.Добавить( "ProductKind",		УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиДокументов.Колонки.Добавить( "Alcohol",			УстановитьТипДанных( Тип("Число"), 15, 4));

	Возврат ТЗСтрокиДокументов;

КонецФункции

Функция ВернутьТЗСтрокиПартий() Экспорт

	ТЗСтрокиПартий = Новый ТаблицаЗначений;
	ТЗСтрокиПартий.Колонки.Добавить( "DocumentDate", 	УстановитьТипДанных( Тип("Дата")));
	ТЗСтрокиПартий.Колонки.Добавить( "DocumentNumber", 	УстановитьТипДанных( Тип("Строка"), 100));  
	ТЗСтрокиПартий.Колонки.Добавить( "Document_Object",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "SrcCRMDbId",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "PartNumber",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "LineNumber",		УстановитьТипДанных( Тип("Число"), 3));
	ТЗСтрокиПартий.Колонки.Добавить( "WareId",			УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиПартий.Колонки.Добавить( "UnitId",			УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиПартий.Колонки.Добавить( "ExpirationDate",	УстановитьТипДанных( Тип("Дата")));
	ТЗСтрокиПартий.Колонки.Добавить( "ProductionDate",	УстановитьТипДанных( Тип("Дата")));
	ТЗСтрокиПартий.Колонки.Добавить( "PalletBarCode",	УстановитьТипДанных( Тип("Строка"), 30));
	ТЗСтрокиПартий.Колонки.Добавить( "Quantity",		УстановитьТипДанных( Тип("Число"), 15, 4));
	ТЗСтрокиПартий.Колонки.Добавить( "FactoryTaxReason",УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "FactoryTaxCode",	УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "FactoryName",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "OriginCountry",	УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "GTDNumber",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISCode",		УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "MultiPackRatio",	УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗСтрокиПартий.Колонки.Добавить( "WareHouseRegId",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "WareHouseBRegId",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "CNLWareId",		УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "CNLWareName",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "WHUTMIdentity",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISAlcCode",	УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISFullName",	УстановитьТипДанных( Тип("Строка"), 500));
	ТЗСтрокиПартий.Колонки.Добавить( "EGAISWbUnitType",	УстановитьТипДанных( Тип("Строка"), 100));
		
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerClientRegId",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerFullName",	УстановитьТипДанных( Тип("Строка"), 500));	
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerShortName",	УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerCountry",		УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerDescription",	УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerINN",			УстановитьТипДанных( Тип("Строка"), 100));
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerKPP",			УстановитьТипДанных( Тип("Строка"), 100));		
	ТЗСтрокиПартий.Колонки.Добавить( "ProducerRegionCode",	УстановитьТипДанных( Тип("Строка"), 100));	
	
	ТЗСтрокиПартий.Колонки.Добавить( "PackageQuantityAL",		УстановитьТипДанных( Тип("Число"), 15, 4));
	ТЗСтрокиПартий.Колонки.Добавить( "EANCodeId",				УстановитьТипДанных( Тип("Строка"), 100));	
	ТЗСтрокиПартий.Колонки.Добавить( "WareHouseAnalytLotIntId",	УстановитьТипДанных( Тип("Число"), 15, 4));
	
	Возврат ТЗСтрокиПартий;

КонецФункции

Функция ВернутьТЗПалет() Экспорт

	ТЗПалет = Новый ТаблицаЗначений; 
	ТЗПалет.Колонки.Добавить( "DocumentDate",		Mon_Общий.УстановитьТипДанных( Тип("Дата")));
	ТЗПалет.Колонки.Добавить( "DocumentNumber",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "Document_Object",	Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "PalletBarCode",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "SrcCRMDbId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "SSCCPalletId",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "WHPalletId",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "PackageQuantity",	Mon_Общий.УстановитьТипДанных( Тип("Число"), 15, 3));
	ТЗПалет.Колонки.Добавить( "WMSPartNumber",		Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	ТЗПалет.Колонки.Добавить( "LineNumber",			Mon_Общий.УстановитьТипДанных( Тип("Число"), 3));	
	ТЗПалет.Колонки.Добавить( "PartNumber",			Mon_Общий.УстановитьТипДанных( Тип("Строка"), 100));
	
	Возврат ТЗПалет;

КонецФункции

Функция ВернутьТекстЗапросаПоступлений() Экспорт

	Текст = "ВЫБРАТЬ
	|	ТЗДокументов.DocumentDate,
	|	ТЗДокументов.DocumentNumber,
	|	ТЗДокументов.WareHouseId,
	|	ТЗДокументов.DocumentBarCode,
	|	ТЗДокументов.AutoNumber,
	|	ТЗДокументов.AutoDriverPersonPassport,
	|	ТЗДокументов.AutoDriverName,
	|	ТЗДокументов.SenderCompanyTaxReason,
	|	ТЗДокументов.SenderCompanyTaxCode,
	|	ТЗДокументов.SenderCompanyName,
	|	ТЗДокументов.SrcDocumentNumber,
	|	ТЗДокументов.MoveDate,
	|	ТЗДокументов.FixNotificationNum,
	|	ТЗДокументов.FixNotificationDate,
	|	ТЗДокументов.IdTTN,
	|	ТЗДокументов.ExtCompanyId,
	|	ТЗДокументов.ExtAddressId
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЗСтрокиДокументов.DocumentDate,
	|	ТЗСтрокиДокументов.DocumentNumber,
	|	ТЗСтрокиДокументов.LineNumber,
	|	ТЗСтрокиДокументов.WareId,
	|	ТЗСтрокиДокументов.UnitId,
	|	ТЗСтрокиДокументов.Price,
	|	ТЗСтрокиДокументов.UnitFactor,
	|	ТЗСтрокиДокументов.Quantity,
	|	ТЗСтрокиДокументов.QuantityL,
	|	ТЗСтрокиДокументов.ProductKind,
	|	ТЗСтрокиДокументов.Alcohol
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЗСтрокиПартий.DocumentDate,
	|	ТЗСтрокиПартий.DocumentNumber,
	|	ТЗСтрокиПартий.PartNumber,
	|	ТЗСтрокиПартий.LineNumber,
	|	ТЗСтрокиПартий.WareId,
	|	ТЗСтрокиПартий.UnitId,
	|	ТЗСтрокиПартий.ExpirationDate,
	|	ТЗСтрокиПартий.ProductionDate,
	|	ТЗСтрокиПартий.PalletBarCode,
	|	ТЗСтрокиПартий.Quantity,
	|	ТЗСтрокиПартий.FactoryTaxReason,
	|	ТЗСтрокиПартий.FactoryTaxCode,
	|	ТЗСтрокиПартий.FactoryName,
	|	ТЗСтрокиПартий.EGAISCode,
	|	ТЗСтрокиПартий.WareHouseRegId,
	|	ТЗСтрокиПартий.WareHouseBRegId,
	|	ТЗСтрокиПартий.CNLWareId,
	|	ТЗСтрокиПартий.CNLWareName,
	|	ТЗСтрокиПартий.WHUTMIdentity,
	|	ТЗСтрокиПартий.ProducerClientRegId,
	|	ТЗСтрокиПартий.PackageQuantityAL
	|ПОМЕСТИТЬ ВТ_ТЗСтрокиПартий
	|ИЗ
	|	&ТЗСтрокиПартий КАК ТЗСтрокиПартий
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Mon_СоответствияНоменклатуры.ТипМонолита КАК ТипМонолита,
	|	Mon_СоответствияНоменклатуры.Номенклатура КАК Номенклатура,
	|	Mon_СоответствияНоменклатуры.Упаковка КАК Упаковка,
	|	Mon_СоответствияНоменклатуры.УпаковкаМонолит КАК УпаковкаМонолит,
	|	Mon_СоответствияНоменклатуры.НоменклатураМонолит КАК НоменклатураМонолит,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДополнительныеСведения.Объект КАК ПриобретениеТоваровУслугСсылка,
	|	ДополнительныеСведения.Значение КАК ПровереноМонолит
	|ПОМЕСТИТЬ ВТ_ПровереноМонолит
	|ИЗ
	|	РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
	|ГДЕ
	|	ДополнительныеСведения.Объект ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|	И ДополнительныеСведения.Свойство.Имя = ""ПровереноМонолит""	
	|;
	|
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТЗСтрокиДокументов.DocumentDate КАК DocumentDate,
	|	ВТ_ТЗСтрокиДокументов.DocumentNumber КАК DocumentNumber,
	|	ВТ_ТЗСтрокиДокументов.LineNumber КАК LineNumber,
	|	ВТ_ТЗСтрокиДокументов.WareId КАК WareId,
	|	ВТ_ТЗСтрокиДокументов.UnitId КАК UnitId,
	|	ВТ_ТЗСтрокиДокументов.Price КАК Price,
	|	ВТ_ТЗСтрокиДокументов.UnitFactor КАК UnitFactor,
	|	ВТ_ТЗСтрокиДокументов.Quantity КАК Quantity,
	|	ВТ_ТЗСтрокиДокументов.QuantityL КАК QuantityL,
	|	ВТ_ТЗСтрокиДокументов.ProductKind КАК ProductKind,
	|	ВТ_ТЗСтрокиДокументов.Alcohol КАК Alcohol,
	|	ВТ_СоответствияНоменклатуры.Номенклатура КАК Номенклатура,
	|	ВТ_СоответствияНоменклатуры.Упаковка КАК Упаковка,
	|	ВТ_СоответствияНоменклатуры.УпаковкаМонолит КАК УпаковкаМонолит,
	|	ВТ_СоответствияНоменклатуры.НоменклатураМонолит КАК НоменклатураМонолит,
	|	ВТ_СоответствияНоменклатуры.Коэффициент КАК Коэффициент,
	|	ВЫБОР
	|		КОГДА ВТ_СоответствияНоменклатуры.Номенклатура ЕСТЬ NULL
	...
	|		КОГДА НЕ ВТ_СоответствияНоменклатуры.Номенклатура ЕСТЬ NULL
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТЗСтрокиПартий.DocumentDate КАК DocumentDate,
	|	ВТ_ТЗСтрокиПартий.DocumentNumber КАК DocumentNumber,
	|	ВТ_ТЗСтрокиПартий.PartNumber КАК PartNumber,
	|	ВТ_ТЗСтрокиПартий.LineNumber КАК LineNumber,
	|	ВТ_ТЗСтрокиПартий.WareId КАК WareId,
	|	ВТ_ТЗСтрокиПартий.UnitId КАК UnitId,
	|	ВТ_ТЗСтрокиПартий.ExpirationDate КАК ExpirationDate,
	|	ВТ_ТЗСтрокиПартий.ProductionDate КАК ProductionDate,
	|	ВТ_ТЗСтрокиПартий.Quantity КАК Quantity,
	|	ВТ_ТЗСтрокиПартий.EGAISCode КАК EGAISCode,
	|	ВТ_ТЗСтрокиПартий.WareHouseRegId КАК WareHouseRegId,
	|	ВТ_ТЗСтрокиПартий.WareHouseBRegId КАК WareHouseBRegId,
	|	ВТ_СоответствияНоменклатуры.Номенклатура КАК Номенклатура,
	|	ВТ_СоответствияНоменклатуры.Упаковка КАК Упаковка,
	|	ВТ_СоответствияНоменклатуры.УпаковкаМонолит КАК УпаковкаМонолит,
	|	ВТ_СоответствияНоменклатуры.НоменклатураМонолит КАК НоменклатураМонолит,
	...
	|		КОГДА ВТ_СоответствияНоменклатуры.Номенклатура ЕСТЬ NULL
	|		ИЛИ ВТ_СоответствияНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА ВТ_ТЗСтрокиПартий.Quantity
	|		КОГДА НЕ ВТ_СоответствияНоменклатуры.Номенклатура ЕСТЬ NULL
	|		И НЕ ВТ_СоответствияНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|		И ВТ_СоответствияНоменклатуры.Упаковка = Фасовка.Значение
	|			ТОГДА ВТ_ТЗСтрокиПартий.Quantity * ВТ_СоответствияНоменклатуры.Коэффициент
	|		КОГДА НЕ ВТ_СоответствияНоменклатуры.Номенклатура ЕСТЬ NULL
	|		И НЕ ВТ_СоответствияНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|		И (ВТ_СоответствияНоменклатуры.Упаковка = Фасовка.Значение
	|		ИЛИ ВТ_СоответствияНоменклатуры.Упаковка = РозливЛитры.Ссылка)
	...
	|		ИЛИ ВТ_СоответствияНоменклатуры.Упаковка = РозливЛитры.Ссылка)
	|		И НЕ ВТ_СоответствияНоменклатуры.УпаковкаМонолит = ""l""
	|			ТОГДА ВТ_ТЗСтрокиПартий.Quantity * ВТ_СоответствияНоменклатуры.Коэффициент / 0.033333333
	|		ИНАЧЕ ВТ_ТЗСтрокиПартий.Quantity
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА ВТ_СоответствияНоменклатуры.Номенклатура ЕСТЬ NULL
	|		ИЛИ ВТ_СоответствияНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	...
	|		ИНАЧЕ ВТ_ТЗСтрокиПартий.Quantity
	|	КОНЕЦ КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ВТ_ТЗСтрокиПартийССоответствиями
	|ИЗ
	|	ВТ_ТЗСтрокиПартий КАК ВТ_ТЗСтрокиПартий
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СоответствияНоменклатуры КАК ВТ_СоответствияНоменклатуры
	|		ПО ВТ_ТЗСтрокиПартий.WareId = ВТ_СоответствияНоменклатуры.НоменклатураМонолит
	|		И ВТ_ТЗСтрокиПартий.UnitId = ВТ_СоответствияНоменклатуры.УпаковкаМонолит
	|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.ЕдиницаИзмеренияКоличестваШтук КАК Фасовка
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.ЕдиницаИзмеренияРазливнойПродукции КАК Розлив
	|		ПО (ИСТИНА)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПриобретениеТоваровУслугСерии.Ссылка КАК Ссылка,
	|	ПриобретениеТоваровУслугСерии.НомерСтроки КАК НомерСтроки,
	...
	|ПОМЕСТИТЬ ВТ_ПриобретениеТоваровУслугСерии
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Серии КАК ПриобретениеТоваровУслугСерии
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТЗДокументов КАК ВТ_ТЗДокументов
	|		ПО ПриобретениеТоваровУслугСерии.Ссылка.НомерВходящегоДокумента = ВТ_ТЗДокументов.DocumentNumber
	|		И ПриобретениеТоваровУслугСерии.Ссылка.ДатаВходящегоДокумента = ВТ_ТЗДокументов.DocumentDate
	|		И (ПриобретениеТоваровУслугСерии.Ссылка.Проведен)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПровереноМонолит КАК ВТ_ПровереноМонолит.ПриобретениеТоваровУслугСсылка
	|		ПО ВТ_ПровереноМонолит.ПриобретениеТоваровУслугСсылка = ПриобретениеТоваровУслугСерии.Ссылка
	|		И  ВТ_ПровереноМонолит.ПровереноМонолит
	|
	|ГДЕ ВТ_ПровереноМонолит.ПриобретениеТоваровУслугСсылка ЕСТЬ NULL
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВТ_ТЗСтрокиДокументовССоответствиями.DocumentDate КАК DocumentDate,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.DocumentNumber КАК DocumentNumber,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.LineNumber КАК LineNumber,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.WareId КАК WareId,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.UnitId КАК UnitId,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Price КАК Price,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.UnitFactor КАК UnitFactor,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Quantity КАК Quantity,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.QuantityL КАК QuantityL,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.ProductKind КАК ProductKind,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Alcohol КАК Alcohol,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Номенклатура КАК Номенклатура,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Упаковка КАК Упаковка,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.УпаковкаМонолит КАК УпаковкаМонолит,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.НоменклатураМонолит КАК НоменклатураМонолит,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Коэффициент КАК Коэффициент,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Количество КАК Количество,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Price * ВТ_ТЗСтрокиДокументовССоответствиями.Quantity КАК Сумма,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Price * ВТ_ТЗСтрокиДокументовССоответствиями.Quantity КАК СуммаСНДС,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Price * ВТ_ТЗСтрокиДокументовССоответствиями.Quantity КАК СуммаВзаиморасчетов,
	|	ВТ_ТЗСтрокиДокументовССоответствиями.Price * ВТ_ТЗСтрокиДокументовССоответствиями.Quantity /
	|		ВТ_ТЗСтрокиДокументовССоответствиями.Количество КАК Цена
	|ПОМЕСТИТЬ ВТ_ТоварыКЗаписи
	|ИЗ
	|	ВТ_ТЗСтрокиДокументовССоответствиями КАК ВТ_ТЗСтрокиДокументовССоответствиями
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ПриобретениеТоваровУслуг
	|		ПО ВТ_ТЗСтрокиДокументовССоответствиями.DocumentDate = ПриобретениеТоваровУслуг.ДатаВходящегоДокумента
	|		И ВТ_ТЗСтрокиДокументовССоответствиями.DocumentNumber = ПриобретениеТоваровУслуг.НомерВходящегоДокумента
	|		И (ПриобретениеТоваровУслуг.Проведен)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПровереноМонолит КАК ВТ_ПровереноМонолит.ПриобретениеТоваровУслугСсылка
	|		ПО ВТ_ПровереноМонолит.ПриобретениеТоваровУслугСсылка = ПриобретениеТоваровУслуг.Ссылка
	|		И  ВТ_ПровереноМонолит.ПровереноМонолит
	|
	|ГДЕ ВТ_ПровереноМонолит.ПриобретениеТоваровУслугСсылка ЕСТЬ NULL
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТЗСтрокиПартийССоответствиями.DocumentDate КАК DocumentDate,
	|	ВТ_ТЗСтрокиПартийССоответствиями.DocumentNumber КАК DocumentNumber,
	|	ВТ_ТЗСтрокиПартийССоответствиями.PartNumber КАК PartNumber,
	|	ВТ_ТЗСтрокиПартийССоответствиями.LineNumber КАК LineNumber,
	|	ВТ_ТЗСтрокиПартийССоответствиями.WareId КАК WareId,
	|	ВТ_ТЗСтрокиПартийССоответствиями.UnitId КАК UnitId,
	|	ВТ_ТЗСтрокиПартийССоответствиями.ExpirationDate КАК ExpirationDate,
	|	ВТ_ТЗСтрокиПартийССоответствиями.ProductionDate КАК ProductionDate,
	|	ВТ_ТЗСтрокиПартийССоответствиями.Quantity КАК Quantity,
	|	ВТ_ТЗСтрокиПартийССоответствиями.EGAISCode КАК EGAISCode,
	|	ВТ_ТЗСтрокиПартийССоответствиями.WareHouseRegId КАК WareHouseRegId,
	|	ВТ_ТЗСтрокиПартийССоответствиями.WareHouseBRegId КАК WareHouseBRegId,
	|	ВТ_ТЗСтрокиПартийССоответствиями.Номенклатура КАК Номенклатура,
	|	ВТ_ТЗСтрокиПартийССоответствиями.Упаковка КАК Упаковка,
	|	ВТ_ТЗСтрокиПартийССоответствиями.УпаковкаМонолит КАК УпаковкаМонолит,
	|	ВТ_ТЗСтрокиПартийССоответствиями.НоменклатураМонолит КАК НоменклатураМонолит,
	|	ВТ_ТЗСтрокиПартийССоответствиями.Коэффициент КАК Коэффициент,
	|	ВТ_ТЗСтрокиПартийССоответствиями.Количество КАК Количество,
	|	ВТ_ТЗСтрокиПартийССоответствиями.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ЕСТЬNULL(ВТ_ПриобретениеТоваровУслугСерииАлко.Серия, ВТ_ПриобретениеТоваровУслугСерииБезАлко.Серия) КАК Серия
	|ПОМЕСТИТЬ ВТ_СерииКЗаписи
	|ИЗ
	|	ВТ_ТЗСтрокиПартийССоответствиями КАК ВТ_ТЗСтрокиПартийССоответствиями
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПриобретениеТоваровУслугСерии КАК ВТ_ПриобретениеТоваровУслугСерииАлко
	|		ПО ВТ_ТЗСтрокиПартийССоответствиями.DocumentDate = ВТ_ПриобретениеТоваровУслугСерииАлко.Ссылка.ДатаВходящегоДокумента
	|		И
	|			ВТ_ТЗСтрокиПартийССоответствиями.DocumentNumber = ВТ_ПриобретениеТоваровУслугСерииАлко.Ссылка.НомерВходящегоДокумента
	|		И
	|			(ВТ_ПриобретениеТоваровУслугСерииАлко.Серия.Справка2ЕГАИС.НомерСправки1 = ВТ_ТЗСтрокиПартийССоответствиями.WareHouseRegId)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПриобретениеТоваровУслугСерии КАК ВТ_ПриобретениеТоваровУслугСерииБезАлко
	|		ПО ВТ_ТЗСтрокиПартийССоответствиями.DocumentDate = ВТ_ПриобретениеТоваровУслугСерииБезАлко.Ссылка.ДатаВходящегоДокумента
	|		И
	|			ВТ_ТЗСтрокиПартийССоответствиями.DocumentNumber = ВТ_ПриобретениеТоваровУслугСерииБезАлко.Ссылка.НомерВходящегоДокумента
	|		И ВТ_ТЗСтрокиПартийССоответствиями.ExpirationDate = ВТ_ПриобретениеТоваровУслугСерииБезАлко.Серия.ГоденДо
	|		И ВТ_ТЗСтрокиПартийССоответствиями.ProductionDate = ВТ_ПриобретениеТоваровУслугСерииБезАлко.Серия.ДатаПроизводства
	|		И (ВТ_ПриобретениеТоваровУслугСерииБезАлко.Серия.Справка2ЕГАИС = ЗНАЧЕНИЕ(СПРАВОЧНИК.Справки2ЕГАИС.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТоварыКЗаписи.Ссылка КАК Ссылка,
	|	ВТ_ТоварыКЗаписи.Склад КАК Склад,
	|	ВТ_ТоварыКЗаписи.DocumentDate КАК DocumentDate,
	|	ВТ_ТоварыКЗаписи.DocumentNumber КАК DocumentNumber,
	|	ВТ_ТоварыКЗаписи.LineNumber КАК LineNumber,
	|	ВТ_ТоварыКЗаписи.WareId КАК WareId,
	|	ВТ_ТоварыКЗаписи.UnitId КАК UnitId,
	|	ВТ_ТоварыКЗаписи.Price КАК Price,
	|	ВТ_ТоварыКЗаписи.UnitFactor КАК UnitFactor,
	|	ВТ_ТоварыКЗаписи.Quantity КАК Quantity,
	|	ВТ_ТоварыКЗаписи.QuantityL КАК QuantityL,
	|	ВТ_ТоварыКЗаписи.ProductKind КАК ProductKind,
	|	ВТ_ТоварыКЗаписи.Alcohol КАК Alcohol,
	|	ВТ_ТоварыКЗаписи.Номенклатура КАК Номенклатура,
	|	ВТ_ТоварыКЗаписи.Упаковка КАК Упаковка,
	|	ВТ_ТоварыКЗаписи.УпаковкаМонолит КАК УпаковкаМонолит,
	|	ВТ_ТоварыКЗаписи.НоменклатураМонолит КАК НоменклатураМонолит,
	|	ВТ_ТоварыКЗаписи.Коэффициент КАК Коэффициент,
	|	ВТ_ТоварыКЗаписи.Количество КАК Количество,
	|	ВТ_ТоварыКЗаписи.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ВТ_ТоварыКЗаписи.Сумма КАК Сумма,
	|	ВТ_ТоварыКЗаписи.СуммаСНДС КАК СуммаСНДС,
	|	ВТ_ТоварыКЗаписи.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
	|	ВТ_ТоварыКЗаписи.Цена КАК Цена,
	|	ВТ_СерииКЗаписи.PartNumber КАК PartNumber,
	|	ВТ_СерииКЗаписи.LineNumber КАК LineNumber1,
	|	ВТ_СерииКЗаписи.WareId КАК WareId1,
	|	ВТ_СерииКЗаписи.UnitId КАК UnitId1,
	|	ВТ_СерииКЗаписи.ExpirationDate КАК ExpirationDate,
	|	ВТ_СерииКЗаписи.ProductionDate КАК ProductionDate,
	|	ВТ_СерииКЗаписи.Quantity КАК Quantity1,
	|	ВТ_СерииКЗаписи.EGAISCode КАК EGAISCode,
	|	ВТ_СерииКЗаписи.WareHouseRegId КАК WareHouseRegId,
	|	ВТ_СерииКЗаписи.WareHouseBRegId КАК WareHouseBRegId,
	|	ВТ_СерииКЗаписи.Номенклатура КАК Номенклатура1,
	|	ВТ_СерииКЗаписи.Упаковка КАК Упаковка1,
	|	ВТ_СерииКЗаписи.УпаковкаМонолит КАК УпаковкаМонолит1,
	|	ВТ_СерииКЗаписи.НоменклатураМонолит КАК НоменклатураМонолит1,
	|	ВТ_СерииКЗаписи.Коэффициент КАК Коэффициент1,
	|	ВТ_СерииКЗаписи.Количество КАК Количество1,
	|	ВТ_СерииКЗаписи.КоличествоУпаковок КАК КоличествоУпаковок1,
	|	ВТ_СерииКЗаписи.Серия КАК Серия
	|ИЗ
	|	ВТ_ТоварыКЗаписи КАК ВТ_ТоварыКЗаписи
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СерииКЗаписи КАК ВТ_СерииКЗаписи
	|		ПО (ВТ_ТоварыКЗаписи.DocumentDate = ВТ_СерииКЗаписи.DocumentDate)
	|		И (ВТ_ТоварыКЗаписи.DocumentNumber = ВТ_СерииКЗаписи.DocumentNumber)
	|		И (ВТ_ТоварыКЗаписи.WareId = ВТ_СерииКЗаписи.WareId)
	|ИТОГИ
	|	МАКСИМУМ(Склад),
	...
	|	МАКСИМУМ(Quantity),
	|	МАКСИМУМ(QuantityL),
	|	МАКСИМУМ(ProductKind),
	|	МАКСИМУМ(Alcohol),
	|	МАКСИМУМ(Номенклатура),
	|	МАКСИМУМ(Упаковка),
	|	МАКСИМУМ(УпаковкаМонолит),
	|	МАКСИМУМ(НоменклатураМонолит),
